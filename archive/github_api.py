...
class GitHubAPI(object):
    ...
    This class is designed to interact with various Github API Endpoints and
    used to construct a repository return object that can then be interacted
    with through the instantiated object. The main purpose of this class is
    to allow an easy method of interacting with repository data without having
    to make repeated calls to Github every time repository data is required.
    """

    class RepoOwner():
        """
        GitHubAPI self class to create a repository owner object that can
        be referenced and used by the parent GitHubAPI class object.
        """
    ...
    def log(self, log_msg, log_type, log_id):
        """
        This class method provides the logging for this class.
        If the class caller instantiates the object with the verbose
        setting set to true, then the class will log to stdout/stderr.
        """
    ...
    def _parse_url(self):
        """
        This class method will accept an HTTP/HTTPS or Git styled URL
        and parse the URL string to extract the repository name and
        repository namespace from the received URL string. The name and
        namespace class properties will then be set so that they will be
        available to both the rest of the class methods as well as
        the class caller as an available object attribute.
        """
        # Define this function for logging
        self.__id = inspect.stack()[0][3]
        self.log(
            f"Call to parse url: {self._target_repo_url}",
            'info',
            self.__id
        )
        ...
        try:
            ...
            self.log(
                "Parsing repository namespace and name from: "
                f"{parse_url_list}",
                'debug',
                self.__id
            )
            ...
            if parse_namespace is not None:
                ...
                self.log(
                    f"Repository namespace: {self.namespace}",
                    'debug',
                    self.__id
                )
            else:
                self.log(
                    "Repository namespace could not be determined. "
                    "Aborting request.",
                    'error',
                    self.__id
                )
            ...
            parse_exc_msg = (
                f"An Exception occurred during method {self.__id} "
                f"exectution: {str(e)} in line: {sys.exc_info()[2].tb_lineno}"
            )
    ...
    def _repository_url(self):
        """
        Class method that accepts the repository namespace and the
        repository name as arguments, and based on those values,
        sets the request URL for the main repository API.
        """
        # Define this function for logging
        self.__id = inspect.stack()[0][3]
        self.log(
            "Constructing API URL for Github Repository API",
            'info',
            self.__id
        )
        ...
        if self.namespace is not None and self.name is not None:
            self._repo_request_url = (
                f"https://api.github.com/repos/{self.namespace}/{self.name}"
            )
            self.log(
                "Constructed Github repository request url: "
                f"{self._repo_request_url}",
                'debug',
                self.__id
            )
        else:
            self.log(
                "Repository namespace or name is not valid, aborting request",
                'error',
                self.__id
            )
            self.log(
    ...
    def _request_handler(self, request_url):
        """
        Class Method to send an http/https request for github repository data
        and validate that the response object is valid, once completed, the
        method will store the response in self.data.
        """
        # Define this function for logging
        self.__id = inspect.stack()[0][3]
        ...
        # Set the request headers and provide
        # auth token if valid instance argument
        request_headers = {}
        if self._auth_token is not None:
            ...
            self.log(
                "Auth token found! Adding to request headers.",
                'debug',
                self.__id
            )
        ...
        try:
            ...
            self.log(
                f"Request successfully sent to: {request_url}.",
                'debug',
                self.__id
            )
            request_handler_response = json.loads(r.text)

            # Validate that the request was successful and if so,
            # then flag the response object for processing
            if r.status_code == 200:
                if isinstance(request_handler_response, dict):
                    if bool(request_handler_response):
                        self.log(
                            f"Response contains return code: {r.status_code}",
                            'debug',
                            self.__id
                        )
                        self.log(
                            "Verified response object contains "
                            "valid formatting.",
                            'debug',
                            self.__id
                        )
                        self.log(
                            "The following response object assigned to "
                            ".data property.",
                            'debug',
                            self.__id
                        )
                        ...
                        self.log(
                            "Empty response object received",
                            'error',
                            self.__id
                        )
                        self.log(
                    ...
                    self.log(
                        "Expected valid JSON response but received "
                        "response object type: "
                        f"{type(request_handler_response)}",
                        'error',
                        self.__id
                    )
                    self.log(
                        "{str(request_handler_response)}",
                        'debug',
                        self.__id
                    )
                    return None
                ...
                self.log(
                    ...
                    f"Received response status code: {r.status_code}",
                    'error',
                    self.__id
                )
                return None
    ...
    def data(self):
        """
        Class getter method used to retrieve the originally stored Github
        response object that is the source used for setting the rest of
        ...
        if (
            self._repo_data is not None and
            isinstance(self._repo_data, dict) and
            bool(self._repo_data)
        ):
    ...
    def data(self, data_source):
        """
        Class setter method that will set the object property values with the
        data received from the request handler self._data dictionary object.
        Normally a setter would be used to set the actual .data object
        property, but in this case we are using the setter to use the
        ._data property as the data passed data source, that is used to
        populate or set all of the object property values based on the
        ._data source values.
        """
        # Define this function for logging
        self.__id = inspect.stack()[0][3]
        ...
        if (
            ...
            isinstance(data_source, dict) and
            bool(data_source)
        ):
            ...
            self.state = 'Success'
            self.log(
                f"Object property update state: {self.state}!",
                'info',
                self.__id
            )
        else:
            ...
            self.log(
                "Updating object properties failed. Bad data source received",
                'error',
                self.__id
            )
            return None
